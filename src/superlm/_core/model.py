...
class RotaryEmbedding(nn.Module):
    def __init__(self, config: ModelConfig) -> None:
        ...
        inv_freq = 1.0 / (config.rope_theta ** (torch.arange(0, head_dim, 2, dtype=torch.int64).float().to(get_device()) / head_dim))
...
class Transformer(GenerationModule):
    ...
    def forward(self, idx: Tensor) -> Tensor:
        pos = torch.arange(0, idx.size()[1], dtype=torch.long, device=get_device()).unsqueeze(0)
