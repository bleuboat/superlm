...
from ..tokenizer import Tokenizer, Streamer
...
class GenerationModule(nn.Module):
    def __init__(self, tokenizer: Tokenizer, **kwargs) -> None:
        ...
        if tokenizer.bos_token_ix >= 0:
            self.generation_config.bos_token_ix = tokenizer.bos_token_ix
        if tokenizer.eos_token_ix >= 0:
            self.generation_config.eos_token_ix = tokenizer.eos_token_ix
        if tokenizer.pad_token_ix >= 0:
            self.generation_config.pad_token_ix = tokenizer.pad_token_ix

    @torch.no_grad()
    def generate(self, inputs: torch.Tensor | None = None, streamer: Streamer | None = None, **kwargs) -> torch.Tensor:
        generation_config = self.generation_config.copy()
        generation_config.update(**kwargs)

        if inputs is None:
            inputs = torch.ones((1, 1), dtype=torch.long, device=get_device()) * generation_config.bos_token_ix
